...
} = require("../models/resetPasswords.js");
const userServices = require("../services/userServices");

// send the back the reset password page
...
router.post("/", async (req, res) => {
  ...
  if (resetToken)
    return res.status(200).send("reset link was sent to this email!");

  try {
    const isLinkSent = await userServices.sendResetPasswordLink(
      user.email,
      user.id
    );

    if (isLinkSent)
      res.status(200).send("reset link has been sent to this email!");
  } catch (e) {
    console.log(e.message);
    return res
      .status(500)
      .send("an internal server was occurred, please try again later!");
  }
});

router.get("/:user_id/:reset_token", async (req, res) => {
  //verification is made inside the resetpassword model
  const isTokenExist = await verifyPasswordResetToken(
    req.params.user_id,
